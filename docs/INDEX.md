...
| **Reports** (BOOTSTRAP étape 7.7) | 2026-06-24 | `superpowers/specs/2026-06-24-reports-design.md` | — | ✅ livré | `ReportEntrypoint` (`$umami->reports`) : CRUD rapports sauvegardés (`list/get/create/update/delete`) + **9 générations** (`funnel/retention/utm/goal/journey/revenue/attribution/performance/breakdown`). Enum `ReportType` (9 types). Réutilise `Filters`/`Period` (zéro nouveau VO). Request `GenerateReport` factorisée (type = path segment). Gardes : `websiteId`/`reportId` non vides, `name`≤200, `description`≤500. Helper `asArray` (passthrough) ajouté. **25 tests unit + 3 intégration**. Quirks live : `filters` requis comme objet (`{}` si vide) ; réponses génération de forme variable (`funnel`→liste, `utm`→objet). |
